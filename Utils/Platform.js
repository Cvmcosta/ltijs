...
class Platform{
     ...
     */
    constructor(name, platform_url, client_id, authentication_endpoint, _ENCRYPTIONKEY){
        ...
        this.platform_url = platform_url || db.get('platforms').find({platform_name: this.platform_name}).value().platform_url
        ...
        if(!db.get('platforms').find({platform_name: this.platform_name}).value()){
            ...
            db.get('platforms').push({platform_name: this.platform_name, platform_url: this.platform_url, client_id: this.client_id, auth_endpoint: this.auth_endpoint}).write()
        }
     ...
     */
    static findPlatform(url, _ENCRYPTIONKEY){
        let ENCRYPTIONKEY = _ENCRYPTIONKEY

        let cryptr = new Cryptr(ENCRYPTIONKEY)
        let adapter = new FileSync('platforms.json', {
            serialize: (data) => cryptr.encrypt(JSON.stringify(data)),
            deserialize: (data) => JSON.parse(cryptr.decrypt(data))
          })
        let db = low(adapter)
        ...
        if(!obj) return false
        return new Platform(obj.platform_name, obj.platform_url, obj.client_id, obj.auth_endpoint, ENCRYPTIONKEY)
