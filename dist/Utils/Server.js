...
class Server {
  constructor(https, ssl, ENCRYPTIONKEY, corsOpt, serverAddon) {
    ...
    this.app.use(async (req, res, next) => {
      ...
      if (req.headers && req.headers.authorization) {
        ...
        if (headerParts.length > 1) {
          ...
          try {
            const tokenBody = headerParts[1]; // Get ltik

            const tokenBodyParts = tokenBody.split(',');
            const ltik = tokenBodyParts[0];
            req.token = ltik; // Get additional Authorization headers
    ...
    this.app.use(async (req, res, next) => {
      ...
      if (req.token) return next(); // Attempt to retrieve ltik from query parameters
