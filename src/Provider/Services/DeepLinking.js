...
class DeepLinking {
  ...
  async createDeepLinkingMessage (idtoken, contentItems, options) {
    ...
    if (!idtoken) { provDeepLinkingDebug('IdToken object missing.'); return false }
    if (!idtoken.platformContext.deepLinkingSettings) { provDeepLinkingDebug('DeepLinkingSettings object missing.'); return false }
    if (!contentItems) { provDeepLinkingDebug('No content item passed.'); return false }
