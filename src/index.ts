...
// `LtiMessageType` (idToken.launch.type) is a real enum, so a real export is required for its
...
// `HttpMethod` is a real enum, not a type-only construct, so a real (non-type-only) export is required
