...
// `MongoLegacyDatabaseManager`'s own validation of what it reads back out of
// Mongo, against its own document shapes -- independent of
// `../mongo/mongo-database-manager.schemas.ts` by design (see this module's
// directory-level isolation convention), even though the shared public
// `PlatformRecord`/`AccessTokenRecord`/`IdTokenRecord` shapes being validated
// against are identical.
