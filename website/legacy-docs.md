...
> **This page documents ltijs v5 and earlier** (the `Provider.setup()` static-singleton API), preserved
> here for reference. If you're starting a new project, see the [current documentation](/guides/getting-started.md)
> instead; if you're upgrading an existing v5 deployment, see [Migrating from ltijs v5](/guides/migrating-from-v5.md).
...
- [Using Ltijs](#using-ltijs)
  ...
  - [Deploy](#deploy)
  - [Platform Class Reference](#platform-class-reference)
...
#### Provider.onSubmissionReview(submissionReviewCallback)
...
| Name | Type | Description |  |
| ---- | ---- | ----------- | -------- |
| submissionReviewCallback | `Function`  | Callback method called everytime a platform sucessfully receives a submission review request. | &nbsp; |


##### Examples

```javascript
lti.onSubmissionReview(async (token, req, res, next) => { return res.send(token) })
```
...
Registers a new [Platform](#platform-class-reference).
...
##### Returns
 ...
 - Promise that resolves a [Platform](#platform-class-reference).
...
Retrieves a [Platform](#platform-class-reference).
...
Deletes a [Platform](#platform-class-reference).
...
Gets all [platforms](#platform-class-reference).



##### Returns

- Promise that resolves a [Platform](#platform-class-reference) object array.
...
<div align="center">
	...
	<img width="300" src="assets/deploy.png"></img>
...
The`lti.registerPlatform()` method returns a Promise that resolves the created [Platform](#platform-class-reference) object.
...
The `lti.getPlatform()` method receives two arguments, `platformUrl` and `clientId`, and returns a Promise that resolves a [Platform](#platform-class-reference) object.

``` javascript
const plat = await lti.getPlatform('http://plat.com', 'CLIENTID') // Returns Platform object
```

*If no **clientId** is passed, this method will return an array containing every [Platform](#platform-class-reference) object that shares the given **platformUrl**.*
...
The `lti.getPlatformById()` method receives the `platformId` and returns a Promise that resolves a [Platform](#platform-class-reference) object.
...
The `lti.getAllPlatforms()` method returns a Promise that resolves an Array containing every registered [Platform](#platform-class-reference).
...
> [Check the Platform Class Documentation](#platform-class-reference)
...
<div align="center">
	...
	<img width="500" src="assets/launch.png"></img>
...
<div align="center">
	...
	<img width="500" src="assets/request.png"></img>
